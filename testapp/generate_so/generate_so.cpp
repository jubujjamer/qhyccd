...
/* Begin live mode
Call this function after setting the stream mode in video mode.*/
uint32_t init_live_mode(qhyccd_handle *pCamHandle){
  uint32_t ret_value;
  ret_value = BeginQHYCCDLive(pCamHandle);
  return ret_value;
}
...
uint32_t set_control_speed(qhyccd_handle *pCamHandle, int cmos_speed){
  uint32_t ret_value = IsQHYCCDControlAvailable(pCamHandle, CONTROL_SPEED);
  if (QHYCCD_SUCCESS == ret_value){
...
/* Getting temperature*/
uint32_t get_temperature(qhyccd_handle *pCamHandle){
  double tmp = GetQHYCCDParam(pCamHandle, CONTROL_CURTEMP);
  return tmp;
}
